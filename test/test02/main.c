...
int Fill(int arrSpaceLens[], int wordNum)
{
    printf(">>>>>>>>>>>>>>>>>>\n");
    printf(">> %d\n", wordNum);
    for (int i = 0; i < spaceCount; i++)
    ...
    for (int i = 0; i < spaceCount; i++)
    {
        ...
        if ((tempSpaceLens[i] == wordLens[wordNum]) && (word2space(spaceLens[i], &words[wordNum][0], &spacesCords[i][0]) == 0))
        {
            ...
            tempSpaceLens[i] = -1;
            returnVal = Fill(tempSpaceLens, tempWordNum);
        ...
        else
        {
            ...
            int flag = 1;
            for (int j = tempWordNum; wordLens[j] == wordLens[wordNum]; j++)
            {
                printf("%d\n", j);
                if ((tempSpaceLens[i] == wordLens[j]) && (word2space(tempSpaceLens[i], &words[j][0], &spacesCords[i][0]) == 0))
                {
                    printf("%d %d\n", j, tempWordNum);
                    flag = 0;
                    temp = tempSpaceLens[i];
                    tempSpaceLens[i] = -1;
                    returnVal = Fill(tempSpaceLens, wordNum);
